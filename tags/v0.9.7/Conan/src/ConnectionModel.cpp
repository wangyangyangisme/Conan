...
namespace conan {
    ...
    */
    ...
    bool ConnectionData::ContainsDuplicateConnections () const {
        ...
        for (std::vector <MethodData>::const_iterator it = mConnections.begin (); it != mConnections.end (); ++it) {
            if (1 < CountConnections (*it)) {
                return true;
            }
        }
        return false;
    ...
    int ConnectionData::CountConnections (const MethodData& inMethod) const {
        return static_cast <int> (
            std::count_if (
                mConnections.begin (),
                mConnections.end (),
                std::bind2nd (std::equal_to <MethodData> (), inMethod)));
    }
