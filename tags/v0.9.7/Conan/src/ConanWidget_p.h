...
#include <corelib/kernel/qmetaobject_p.h>
        ...
        void BuildSlotData (const QObject* inObject, std::vector <ConnectionData>& outConnections) {
            ...
            try {
                if (QObjectPrivate* object_p = QObjectPrivate::get (const_cast <QObject*> (inObject))) {
                    ...
                    std::set <std::pair <QObject*, int> > processedConnections;  // set <sender, slotIndex>
                    ...
                    for (QObjectPrivate::Connection *s = object_p->senders; s; s = s->next) {
                        ...
                        if (processedConnections.find(std::make_pair(s->sender, connectionIndex)) != processedConnections.end())
                            // we already processed all connections for this sender to the slot
                            continue;
                        processedConnections.insert(std::make_pair(s->sender, connectionIndex));
                        ...
                        for (size_t c = 0; c < senderConnections.size (); c++) {
                            ...
                            for (size_t s = 0; s < connectionData.mConnections.size (); s++) {
