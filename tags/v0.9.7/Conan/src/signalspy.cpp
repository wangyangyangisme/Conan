...
namespace conan {
    ...
    */
        ...
        mEmitCount (0)
    {
        ...
        assert (inObject && inLogger);

        const QMetaObject* metaObject = inObject->metaObject ();
        int sigIndex = metaObject->indexOfMethod (inSignal.toStdString ().c_str ());
        if (sigIndex < 0) {
            qWarning ("SignalSpy: No such signal: '%s'", inSignal.toStdString ().c_str ());
            return;
        }
    ...
    */
    void SignalLogger::Log (SignalSpy* inSpy, const QString& inArgs) const {
    ...
    */
    void SignalSpyModel::CreateSignalSpy (const QObject* inObject, const QString& inSignal) {
        ...
        std::auto_ptr <SignalSpy> spy (new SignalSpy (inObject, inSignal, &mLogger));
        if (spy->SignalData ().mObject) {
            mLogger.mObjectWidth = std::max <unsigned> (mLogger.mObjectWidth, ObjectUtility::QualifiedName (spy->SignalData ().mClass, spy->SignalData ().mName).count ());
            mLogger.mSignatureWidth = std::max <unsigned> (mLogger.mSignatureWidth, spy->SignalData ().mSignature.count ());

            beginInsertRows (QModelIndex (), rowCount (), rowCount ());
            mSignalSpies.push_back (spy.release ());
            endInsertRows ();
        }
    }
    ...
    */
    void SignalSpyModel::DestroySignalSpy (const QObject* inObject, const QString& inSignal) {
        for (std::vector <SignalSpy*>::iterator it=mSignalSpies.begin (); it != mSignalSpies.end (); ++it) {
            SignalSpy* spy = *it;
            if (SameSignal (inObject, inSignal, spy)) {
                int row = std::distance (mSignalSpies.begin (), it);
                removeRows (row, 1);
                break;
            }
        }
    ...
    /*!
        \brief Updates the logger options to the given values.
    */
    void SignalSpyModel::SetLogOptions (
        bool inShowTimestamp, bool inShowObject, bool inShowAddress,
        bool inShowSignature, bool inShowEmitCount, bool inShowArguments,
        bool inPrettyFormatting, QChar inSeparator)
    {
        mLogger.mShowTimestamp = inShowTimestamp;
        mLogger.mShowObject = inShowObject;
        mLogger.mShowAddress = inShowAddress;
        mLogger.mShowSignature = inShowSignature;
        mLogger.mShowEmitCount = inShowEmitCount;
        mLogger.mShowArguments = inShowArguments;
        mLogger.mPrettyFormatting = inPrettyFormatting;
        mLogger.mSeparator = inSeparator;
    }

    /*!
        \brief Returns an example signal spy log based on the current log options.
    */
    QString SignalSpyModel::GetExampleLog () const {
         return mLogger.GetExampleLog ();
    }
    ...
    */
    bool SignalSpyModel::removeRows (int inRow, int inCount, const QModelIndex& inParent) {
        ...
        std::vector <SignalSpy*>::iterator first = mSignalSpies.begin ();
        std::advance (first, inRow);
        std::vector <SignalSpy*>::iterator last = first;
        std::advance (last, inCount);
