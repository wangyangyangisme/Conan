...
namespace conan {
			...
			virtual void undo () {
				if (mSelectionModel && !mSkipFirstSelect) {
					if (const ObjectModel* model = dynamic_cast <const ObjectModel*> (mSelectionModel->model ())) {
						QModelIndex objectIndex = model->FindObject (mNewAddress);
    ...
    */
	bool ConanWidget::FindAndSelectObject (const QString& inValue) {
		if (inValue.isEmpty ()) {
			return true;
		}
		QModelIndex current = mForm.objectTree->currentIndex ();
		QModelIndex result = mObjectModel->FindObject (inValue, current);
		if (result.isValid ()) {
    ...
    */
	void ConanWidget::SlotFindMethod (const QModelIndex& inIndex) {
		...
		if (!FindAndSelectObject (methodCopy.mAddress)) {
