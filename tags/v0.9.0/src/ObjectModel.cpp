...
namespace conan {
    ...
    */
    int ObjectItem::ChildIndex (const ObjectItem* inChild) const {
        ...
        int index = std::distance (mChildren.begin (), std::find (mChildren.begin (), mChildren.end (), inChild));
    ...
    */
    QString ObjectItem::Class () const {
        return mClass;
    }
    ...
    */
    const ObjectItem* ObjectItem::Find (const QString& inValue, const ObjectItem* inStartItem, bool* inActive) const {
        ...
        foreach (const ObjectItem* item, mChildren) {
            if (!inActive || (*inActive)) {
                if (item->Name ().contains (inValue, Qt::CaseInsensitive) ||
                    item->Class ().contains (inValue, Qt::CaseInsensitive) ||
                    item->Address ().contains (inValue, Qt::CaseInsensitive))
                {
                    return item;
                }
            }
            // recurse
            if (const ObjectItem* child = item->Find (inValue, inStartItem, inActive)) {
                return child;
            }
        }
        return 0;
    ...
    */
    void ObjectItem::Sort (const ObjectItemSorter& inPred) {
        std::sort (mChildren.begin (), mChildren.end (), inPred);
    ...
    */
    void ObjectModel::SlotRefresh () {
        ...
        std::vector <const QObject*> rootObjects;
        int childCount = mRoot->ChildCount ();
        for (int c=0; c<childCount; c++) {
            const ObjectItem* item = mRoot->Child (c);
            if (const QObject* object = item->Object ()) {
    ...
    */
    void ObjectModel::SlotSort () {
        ...
        foreach (QModelIndex index, persistentIndices) {
            const QObject* object = object = GetObject (index);
        ...
        foreach (QModelIndex from, persistentIndices) {
            QModelIndex to = FindObject (addresses.front ());
    ...
    */
    ...
    /*!
        ...
        \param[in] inValue  The value to match each object with
        \param[in] inStart  The item to start searching from (note that the search wraps)
        \return             The item that wraps the found object
    */
    QModelIndex ObjectModel::FindObject (const QString& inValue, const QModelIndex& inStart) const {
        const ObjectItem* startItem = GetItem (inStart);
        bool active = false;
        // find starting from startItem
        const ObjectItem* result = mRoot->Find (inValue, startItem, &active);
        if (!result) {
            // wrap search
            active = true;
            // find untill startItem
            result = mRoot->Find (inValue, startItem, &active);
        }
