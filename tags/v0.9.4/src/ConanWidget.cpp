...
namespace conan {
    ...
    */
    void ConanWidget::SlotConnectionContextMenuRequested (const QPoint& inPos) {
        ...
        QPointer <QObject> object = methodData ? methodData->mObject : const_cast <QObject*> (mObjectModel->GetObject (objectIndex));
    ...
    */
    void ConanWidget::SlotDeleteSpies () {
        while (mForm.signalSpiesTableView->selectionModel ()->hasSelection ()) {
            QItemSelection selection = mForm.signalSpiesTableView->selectionModel ()->selection ();
